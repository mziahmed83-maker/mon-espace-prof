...
import { BookOpen, ClipboardList, ExternalLink, FileText, GraduationCap, Home, Link as LinkIcon, LogOut, Plus, Save, Upload, Users } from 'lucide-react'
...
const initialClasses: Classe[] = [{id:'c1',nom:'TC 1',niveau:'Tronc commun'},{id:'c2',nom:'3e A',niveau:'3e collège'}]
const initialEleves: Eleve[] = [{id:'e1',nom:'Amrani',prenom:'Sara',classeId:'c1'},{id:'e2',nom:'Bennani',prenom:'Yassine',classeId:'c1'},{id:'e3',nom:'El Fassi',prenom:'Lina',classeId:'c2'}]
const initialNotes: Note[] = [{id:'n1',eleveId:'e1',libelle:'Contrôle 1',valeur:15,sur:20,coefficient:1},{id:'n2',eleveId:'e2',libelle:'Contrôle 1',valeur:12.5,sur:20,coefficient:1},{id:'n3',eleveId:'e3',libelle:'Contrôle 1',valeur:17,sur:20,coefficient:1}]
const initialDevoirs: Devoir[] = [{id:'d1',titre:'Lire Aux champs + questions',classeId:'c1',date:'2026-09-10',statut:'À faire'},{id:'d2',titre:'Figures de style — exercices',classeId:'c2',date:'2026-09-08',statut:'À faire'}]
...
export default function App(){
  ...
  const nav=[['dashboard','Accueil',Home],['classes','Classes',GraduationCap],['eleves','Élèves',Users],['notes','Notes',ClipboardList],['devoirs','Devoirs',BookOpen],['documents','Mes documents',FileText]] as const
  ...
  return <div className="shell">
    <aside className="sidebar"><div className="brand">Mon Espace Prof<small>Gestion pédagogique personnelle</small></div><div className="nav">{nav.map(([k,label,Icon])=><button key={k} onClick={()=>setTab(k)} className={tab===k?'active':''}><Icon size={18}/>{label}</button>)}</div></aside>
    <main className="main">
      <div className="top"><div><h1>{nav.find(x=>x[0]===tab)?.[1]}</h1><p>Votre espace de travail enseignant, simple et centralisé.</p></div>{user?<button className="btn secondary" onClick={()=>supabase.auth.signOut()}><LogOut size={15}/> Se déconnecter</button>:<span className="pill">Connexion requise pour les fichiers</span>}</div>
      {tab==='dashboard'&&<><div className="cards"><Stat label="Classes" value={classes.length}/><Stat label="Élèves" value={eleves.length}/><Stat label="Devoirs à faire" value={devoirs.filter(d=>d.statut==='À faire').length}/><Stat label="Moyenne générale" value={`${moyenne.toFixed(1)}/20`}/></div><div className="grid2"><section className="panel"><h2>Derniers devoirs</h2><div className="list">{devoirs.slice(0,5).map(d=><div className="item" key={d.id}><strong>{d.titre}</strong><span>{classeNom(d.classeId)} · {d.date}</span></div>)}</div></section><section className="panel"><h2>Bibliothèque</h2><div className="list">{docs.slice(0,5).map(d=><div className="item" key={d.id}><strong>{d.titre}</strong><span>{d.categorie} · {d.niveau}</span></div>)}</div></section></div></>}
...
function Stat({label,value}:{label:string,value:string|number}){return <div className="card"><div className="label">{label}</div><div className="value">{value}</div></div>}
