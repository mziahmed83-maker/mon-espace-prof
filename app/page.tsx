...
type Preferences = {nom:string;fonction:string;sousTitre:string;citation:string;accent:string;theme:'clair'|'sombre'}
...
const defaultPreferences:Preferences={nom:'M. Ahmed',fonction:'Professeur de français',sousTitre:'Gestion pédagogique personnelle',citation:'Transmettre, c’est semer des possibles.',accent:'#1478ff',theme:'clair'}
...
export default function App(){
  ...
  const [preferences,setPreferences]=useStored<Preferences>('ep-preferences',defaultPreferences)
  ...
  useEffect(()=>{
    try{
      if(localStorage.getItem('ep-exemples-nettoyes-v1')) return
      const sampleStudentIds=new Set(['e1','e2','e3'])
      setClasses(prev=>prev.filter(c=>!(c.id==='c2'||c.nom.trim().toLowerCase()==='3e a')))
      setEleves(prev=>prev.filter(e=>!sampleStudentIds.has(e.id)&&!['sara amrani','yassine bennani','lina el fassi'].includes(`${e.prenom} ${e.nom}`.trim().toLowerCase())))
      setNotes(prev=>prev.filter(n=>!sampleStudentIds.has(n.eleveId)))
      setDevoirs(prev=>prev.filter(d=>d.classeId!=='c2'))
      localStorage.setItem('ep-exemples-nettoyes-v1','1')
    }catch{}
  },[setClasses,setEleves,setNotes,setDevoirs])
  ...
  const nav=[['dashboard','Accueil',Home],['classes','Classes',GraduationCap],['eleves','Élèves',Users],['notes','Notes',ClipboardList],['devoirs','Devoirs',BookOpen],['documents','Mes documents',FolderOpen],['bibliotheque','Bibliothèque',Library],['calendrier','Mon calendrier',CalendarDays],['statistiques','Statistiques',BarChart3],['parametres','Paramètres',Settings]] as const
  ...
  async function openDocument(d:DocumentPedago){if(d.storagePath){const {data,error}=await supabase.storage.from('documents-pedago').createSignedUrl(d.storagePath,3600);if(error){alert('Impossible d’ouvrir ce fichier : '+error.message);return}window.open(data.signedUrl,'_blank','noopener,noreferrer');return}if(d.lien)window.open(d.lien,'_blank','noopener,noreferrer')}

  function editClasse(c:Classe){const nom=window.prompt('Nom de la classe :',c.nom);if(nom===null||!nom.trim())return;const niveau=window.prompt('Niveau :',c.niveau);if(niveau===null||!niveau.trim())return;setClasses(classes.map(x=>x.id===c.id?{...x,nom:nom.trim(),niveau:niveau.trim()}:x))}
  function deleteClasse(c:Classe){const linked=eleves.filter(e=>e.classeId===c.id);const message=linked.length?`Supprimer « ${c.nom} » ? Les ${linked.length} élève(s) rattaché(s) à cette classe et leurs notes seront aussi supprimés.`:`Supprimer la classe « ${c.nom} » ?`;if(!window.confirm(message))return;const studentIds=new Set(linked.map(e=>e.id));setClasses(classes.filter(x=>x.id!==c.id));setEleves(eleves.filter(e=>e.classeId!==c.id));setNotes(notes.filter(n=>!studentIds.has(n.eleveId)));setDevoirs(devoirs.filter(d=>d.classeId!==c.id))}
  function editEleve(e:Eleve){const nom=window.prompt('Nom :',e.nom);if(nom===null||!nom.trim())return;const prenom=window.prompt('Prénom :',e.prenom);if(prenom===null||!prenom.trim())return;const choices=classes.map(c=>`${c.nom} (${c.niveau})`).join('\n');const classe=window.prompt(`Classe actuelle : ${classeNom(e.classeId)}\n\nEntrez le nom exact de la nouvelle classe.\nClasses disponibles :\n${choices}`,classeNom(e.classeId));if(classe===null)return;const selected=classes.find(c=>c.nom.trim().toLowerCase()===classe.trim().toLowerCase());setEleves(eleves.map(x=>x.id===e.id?{...x,nom:nom.trim(),prenom:prenom.trim(),classeId:selected?.id??e.classeId}:x))}
  function deleteEleve(e:Eleve){if(!window.confirm(`Supprimer ${e.prenom} ${e.nom} de la liste des élèves ?`))return;setEleves(eleves.filter(x=>x.id!==e.id));setNotes(notes.filter(n=>n.eleveId!==e.id))}
  function editNote(n:Note){const libelle=window.prompt('Évaluation :',n.libelle);if(libelle===null||!libelle.trim())return;const valeur=window.prompt('Note obtenue :',String(n.valeur));if(valeur===null||Number.isNaN(Number(valeur)))return;const sur=window.prompt('Note sur :',String(n.sur));if(sur===null||Number.isNaN(Number(sur)))return;const coefficient=window.prompt('Coefficient :',String(n.coefficient));if(coefficient===null||Number.isNaN(Number(coefficient)))return;setNotes(notes.map(x=>x.id===n.id?{...x,libelle:libelle.trim(),valeur:Number(valeur),sur:Number(sur),coefficient:Number(coefficient)}:x))}
  ...
  function editDevoir(d:Devoir){const titre=window.prompt('Titre / consigne :',d.titre);if(titre===null||!titre.trim())return;const date=window.prompt('Date (AAAA-MM-JJ) :',d.date);if(date===null||!date.trim())return;const statut=window.prompt('Statut : À faire ou Terminé',d.statut);if(statut===null)return;const nextStatut:Devoir['statut']=statut.trim().toLowerCase().startsWith('term')?'Terminé':'À faire';setDevoirs(devoirs.map(x=>x.id===d.id?{...x,titre:titre.trim(),date:date.trim(),statut:nextStatut}:x))}
  function deleteDevoir(d:Devoir){if(window.confirm(`Supprimer le devoir « ${d.titre} » ?`))setDevoirs(devoirs.filter(x=>x.id!==d.id))}
  function editDocument(d:DocumentPedago){const titre=window.prompt('Titre du document :',d.titre);if(titre===null||!titre.trim())return;const categorie=window.prompt('Catégorie :',d.categorie);if(categorie===null||!categorie.trim())return;const niveau=window.prompt('Niveau :',d.niveau);if(niveau===null||!niveau.trim())return;let lien=d.lien;if(d.source!=='pc'){const nouveauLien=window.prompt(d.source==='drive'?'Lien Google Drive :':'Lien du document :',d.lien??'');if(nouveauLien===null)return;lien=nouveauLien.trim()}setDocs(docs.map(x=>x.id===d.id?{...x,titre:titre.trim(),categorie:categorie.trim(),niveau:niveau.trim(),lien}:x))}
  async function deleteDocument(d:DocumentPedago){if(!window.confirm(`Supprimer le document « ${d.titre} » ?`))return;if(d.storagePath){if(!user){alert('Reconnectez-vous avant de supprimer un fichier stocké dans Supabase.');return}const {error}=await supabase.storage.from('documents-pedago').remove([d.storagePath]);if(error){alert('Impossible de supprimer le fichier : '+error.message);return}}setDocs(docs.filter(x=>x.id!==d.id))}

  async function saveItem(data:any){const id=crypto.randomUUID();if(modal==='classe')setClasses([...classes,{id,...data}]);if(modal==='eleve')setEleves([...eleves,{id,...data}]);if(modal==='note')setNotes([...notes,{id,...data,valeur:Number(data.valeur),sur:Number(data.sur),coefficient:Number(data.coefficient)}]);if(modal==='devoir')setDevoirs([...devoirs,{id,...data}]);if(modal==='document'){if(data.source==='pc'){if(!user)throw new Error('Connectez-vous d’abord avec votre e-mail pour téléverser un fichier.');const file:File|undefined=data.fichier;if(!file)throw new Error('Choisissez un fichier à téléverser.');const safeName=file.name.replace(/[^a-zA-Z0-9._-]/g,'_');const path=`${user.id}/${crypto.randomUUID()}-${safeName}`;const {error}=await supabase.storage.from('documents-pedago').upload(path,file,{contentType:file.type||undefined,upsert:false});if(error)throw error;const clean={...data};delete clean.fichier;setDocs([...docs,{id,...clean,fichierNom:file.name,storagePath:path}])}else{if(!data.lien?.trim())throw new Error('Ajoutez le lien du document.');setDocs([...docs,{id,...data,lien:data.lien.trim()}])}}setModal(null)}
  ...
  const appStyle={{'--accent':preferences.accent} as React.CSSProperties}

  return <div className={`app-frame theme-${preferences.theme}`} style={appStyle}>
    <header className="app-header">
      <div className="header-brand"><div className="logo-mark">📘</div><div><strong>Mon Espace Prof</strong><small>{preferences.sousTitre}</small></div></div>
      ...
      <div className="profile-zone"><button className="icon-button" aria-label="Notifications"><Bell size={20}/><span className="notify-dot"/></button><div className="avatar">{initials}</div><div className="profile-copy"><strong>{preferences.nom}</strong><span>{preferences.fonction}</span></div></div>
    </header>
    <div className="shell">
      ...
      <main className="main">
        {tab==='dashboard'?<>
          <section className="welcome-row"><div><div className="welcome-title"><Sun size={38}/><div><h1>Bienvenue, {preferences.nom} !</h1><p>Votre espace de travail enseignant, simple et centralisé.</p></div></div></div><blockquote>« {preferences.citation} »</blockquote></section>
          <div className="cards"><Stat label="Classes" value={classes.length} icon={<GraduationCap size={30}/>} onClick={()=>setTab('classes')}/><Stat label="Élèves" value={eleves.length} icon={<Users size={30}/>} onClick={()=>setTab('eleves')}/><Stat label="Devoirs à faire" value={devoirs.filter(d=>d.statut==='À faire').length} icon={<FileText size={30}/>} onClick={()=>setTab('devoirs')}/><Stat label="Moyenne générale" value={moyenne.toFixed(1)} icon={<BarChart3 size={30}/>} onClick={()=>setTab('notes')}/></div>
          <div className="grid2 dashboard-grid"><section className="panel modern-panel"><div className="panel-heading"><h2><FileText size={20}/> Derniers devoirs</h2><button className="text-link" onClick={()=>setTab('devoirs')}>Voir tout</button></div><div className="list">{devoirs.slice(0,5).map((d,i)=><div className="item rich-item" key={d.id}><div className={`item-icon icon-${i%3}`}><FileText size={20}/></div><div className="item-body"><strong>{d.titre}</strong><span>{classeNom(d.classeId)} · {d.date}</span></div><span className={d.statut==='À faire'?'status-chip todo':'status-chip done'}>{d.statut==='À faire'?'À corriger':'Corrigé'}</span><ChevronRight size={20}/></div>)}</div></section><section className="panel modern-panel"><div className="panel-heading"><h2><Library size={20}/> Bibliothèque</h2><button className="text-link" onClick={()=>setTab('bibliotheque')}>Voir tout</button></div><div className="list">{filteredDocs.slice(0,5).map((d,i)=><button className="item rich-item library-row" key={d.id} onClick={()=>openDocument(d)}><div className={`item-icon lib-${i%3}`}><BookOpen size={20}/></div><div className="item-body"><strong>{d.titre}</strong><span>{d.categorie} · {d.niveau}</span></div><ChevronRight size={20}/></button>)}</div></section></div>
          ...
          {tab==='parametres'&&<SettingsPanel preferences={preferences} onChange={setPreferences}/>} 
        </>}
      </main>
    </div>
...
function SettingsPanel({preferences,onChange}:{preferences:Preferences;onChange:(p:Preferences)=>void}){const set=(key:keyof Preferences,value:string)=>onChange({...preferences,[key]:value} as Preferences);const colors=['#1478ff','#7c3aed','#db2777','#059669','#ea580c','#0f766e'];return <section className="panel settings-panel"><div className="settings-title"><div className="coming-icon"><Settings size={32}/></div><div><h2>Personnaliser Mon Espace Prof</h2><p>Vos choix sont appliqués immédiatement et enregistrés automatiquement.</p></div></div><div className="settings-grid"><label>Nom affiché<input className="input" value={preferences.nom} onChange={e=>set('nom',e.target.value)}/></label><label>Fonction<input className="input" value={preferences.fonction} onChange={e=>set('fonction',e.target.value)}/></label><label>Sous-titre de l’application<input className="input" value={preferences.sousTitre} onChange={e=>set('sousTitre',e.target.value)}/></label><label>Phrase d’accueil<input className="input" value={preferences.citation} onChange={e=>set('citation',e.target.value)}/></label></div><div className="setting-block"><strong>Couleur principale</strong><div className="color-choices">{colors.map(c=><button key={c} className={`color-choice ${preferences.accent===c?'selected':''}`} style={{background:c}} onClick={()=>set('accent',c)} aria-label={`Couleur ${c}`}/>)}</div></div><div className="setting-block"><strong>Thème</strong><div className="theme-choices"><button className={preferences.theme==='clair'?'selected':''} onClick={()=>set('theme','clair')}>☀️ Clair</button><button className={preferences.theme==='sombre'?'selected':''} onClick={()=>set('theme','sombre')}>🌙 Sombre</button></div></div><button className="btn secondary reset-btn" onClick={()=>onChange(defaultPreferences)}>Réinitialiser l’apparence</button></section>}
...
function Editor({type,classes,eleves,onClose,onSave}:{type:'classe'|'eleve'|'note'|'devoir'|'document';classes:Classe[];eleves:Eleve[];onClose:()=>void;onSave:(d:any)=>Promise<void>}){
  ...
  const [saving,setSaving]=useState(false);const [error,setError]=useState('');const f=(k:string,v:any)=>setForm((p:any)=>({...p,[k]:v}));const title={classe:'Ajouter une classe',eleve:'Ajouter un élève',note:'Ajouter une note',devoir:'Ajouter un devoir',document:'Téléverser / ajouter un document'}[type]
  ...
  return <div className="modal-backdrop"><div className="modal"><h3>{title}</h3><div className="form">{type==='classe'&&<><Field label="Nom de la classe" value={form.nom} onChange={v=>f('nom',v)}/><Field label="Niveau" value={form.niveau} onChange={v=>f('niveau',v)}/></>}{type==='eleve'&&<><Field label="Nom" value={form.nom} onChange={v=>f('nom',v)}/><Field label="Prénom" value={form.prenom} onChange={v=>f('prenom',v)}/><Select label="Classe" value={form.classeId} onChange={v=>f('classeId',v)} options={classes.map(c=>[c.id,c.nom])}/></>}{type==='note'&&<><Select label="Élève" value={form.eleveId} onChange={v=>f('eleveId',v)} options={eleves.map(e=>[e.id,`${e.prenom} ${e.nom}`])}/><Field label="Évaluation" value={form.libelle} onChange={v=>f('libelle',v)}/><Field label="Note" type="number" value={form.valeur} onChange={v=>f('valeur',v)}/><Field label="Sur" type="number" value={form.sur} onChange={v=>f('sur',v)}/><Field label="Coefficient" type="number" value={form.coefficient} onChange={v=>f('coefficient',v)}/></>}{type==='devoir'&&<><Field label="Titre / consigne" value={form.titre} onChange={v=>f('titre',v)}/><Select label="Classe" value={form.classeId} onChange={v=>f('classeId',v)} options={classes.map(c=>[c.id,c.nom])}/><Field label="Date" type="date" value={form.date} onChange={v=>f('date',v)}/><Select label="Statut" value={form.statut} onChange={v=>f('statut',v)} options={['À faire','Terminé'].map(x=>[x,x])}/></>}{type==='document'&&<><Field label="Titre du document" value={form.titre} onChange={v=>f('titre',v)}/><Field label="Catégorie" value={form.categorie} onChange={v=>f('categorie',v)}/><Field label="Niveau" value={form.niveau} onChange={v=>f('niveau',v)}/><Select label="Source" value={form.source} onChange={v=>f('source',v)} options={[["pc","Depuis mon PC"],["drive","Google Drive"],["lien","Autre lien"]]}/>{form.source==='pc'?<label>Choisir un fichier<input className="input" type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.jpg,.jpeg,.png" onChange={e=>{const file=e.target.files?.[0];f('fichier',file);f('fichierNom',file?.name??'')}}/>{form.fichierNom&&<span className="file-picked"><Upload size={14}/> {form.fichierNom}</span>}</label>:<Field label={form.source==='drive'?'Lien Google Drive':'Lien du document'} value={form.lien} onChange={v=>f('lien',v)} placeholder="https://..."/>}</>}{error&&<div className="auth-message">{error}</div>}<div className="actions"><button className="btn secondary" onClick={onClose} disabled={saving}>Annuler</button><button className="btn" onClick={submit} disabled={saving}><Save size={16}/> {saving?'Téléversement…':'Enregistrer'}</button></div></div></div></div>
}
