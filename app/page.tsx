...
export default function App(){
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand">Mon Espace Prof<small>Gestion pédagogique personnelle</small></div>
      <div className="nav">{nav.map(([k,label,Icon])=><button key={k} onClick={()=>setTab(k)} className={tab===k?'active':''}><Icon size={18}/>{label}</button>)}</div>
    </aside>
    <main className="main">
      <div className="top"><div><h1>{nav.find(x=>x[0]===tab)?.[1]}</h1><p>Votre espace de travail enseignant, simple et centralisé.</p></div><span className="pill">Mode démo local</span></div>
      {tab==='dashboard' && <>
        <div className="cards">
          <Stat label="Classes" value={classes.length}/><Stat label="Élèves" value={eleves.length}/><Stat label="Devoirs à faire" value={devoirs.filter(d=>d.statut==='À faire').length}/><Stat label="Moyenne générale" value={`${moyenne.toFixed(1)}/20`}/>
        </div>
        <div className="grid2"><section className="panel"><h2>Derniers devoirs</h2><div className="list">{devoirs.slice(0,5).map(d=><div className="item" key={d.id}><strong>{d.titre}</strong><span>{classeNom(d.classeId)} · {d.date}</span></div>)}</div></section><section className="panel"><h2>Bibliothèque</h2><div className="list">{docs.slice(0,5).map(d=><div className="item" key={d.id}><strong>{d.titre}</strong><span>{d.categorie} · {d.niveau}</span></div>)}</div></section></div>
      </>}
    ...
    {modal && <Editor type={modal} classes={classes} eleves={eleves} onClose={()=>setModal(null)} onSave={(data:any)=>{
      ...
      if(modal==='document') setDocs([...docs,{id,...data}]);
      setModal(null)
    }}/>} 
  </div>
}
