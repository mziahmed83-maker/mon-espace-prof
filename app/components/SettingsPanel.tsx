...
export default function SettingsPanel({preferences,onPreferences,rubriqueNoms,onRubriqueNoms,customRubriques,onCustomRubriques}:{preferences:Preferences;onPreferences:(p:Preferences)=>void;rubriqueNoms:RubriqueNoms;onRubriqueNoms:(r:RubriqueNoms)=>void;customRubriques:CustomRubrique[];onCustomRubriques:(r:CustomRubrique[])=>void}){
  ...
  const setPref=<K extends keyof Preferences>(key:K,value:Preferences[K])=>{
    ...
    if(key==='titleColor')document.documentElement.style.setProperty('--title-color',String(value))
    if(key==='subtitleColor')document.documentElement.style.setProperty('--subtitle-color',String(value))
  }
  ...
  return <section className="panel settings-panel">
    ...
    <div className="settings-grid"><label>Titre de l’application<input className="input" value={preferences.titreApp} onChange={e=>setPref('titreApp',e.target.value)}/></label><label>Sous-titre de l’application<input className="input" value={preferences.sousTitre} onChange={e=>setPref('sousTitre',e.target.value)}/></label><label>Nom affiché<input className="input" value={preferences.nom} onChange={e=>setPref('nom',e.target.value)}/></label><label>Fonction<input className="input" value={preferences.fonction} onChange={e=>setPref('fonction',e.target.value)}/></label><label>Phrase d’accueil<input className="input" value={preferences.citation} onChange={e=>setPref('citation',e.target.value)}/></label></div>
    ...
    <div className="setting-block"><strong>Ajouter une nouvelle rubrique</strong><div className="settings-grid"><label>Titre<input className="input" value={newTitle} onChange={e=>setNewTitle(e.target.value)} placeholder="Ex. Réunions"/></label><label>Sous-titre<input className="input" value={newSub} onChange={e=>setNewSub(e.target.value)} placeholder="Ex. Notes et comptes rendus"/></label></div><button className="btn" style={{marginTop:12}} onClick={addRubrique}><Plus size={16}/> Ajouter la rubrique</button>{customRubriques.length>0&&<div className="table-wrap" style={{marginTop:16}}><table className="table"><thead><tr><th>Rubrique</th><th>Sous-titre</th><th>Actions</th></tr></thead><tbody>{customRubriques.map(r=><tr key={r.id}><td><b>{r.titre}</b></td><td>{r.sousTitre}</td><td><div className="row-actions"><button className="action-btn edit" onClick={()=>editRubrique(r)}><Pencil size={15}/> Modifier</button><button className="action-btn delete" onClick={()=>deleteRubrique(r)}><Trash2 size={15}/> Supprimer</button></div></td></tr>)}</tbody></table></div>}</div>
    <button className="btn secondary reset-btn" onClick={()=>{onPreferences(defaultPreferences);onRubriqueNoms(defaultRubriqueNoms);document.documentElement.style.setProperty('--title-color',defaultPreferences.titleColor);document.documentElement.style.setProperty('--subtitle-color',defaultPreferences.subtitleColor)}}>Réinitialiser l’apparence</button>
  </section>
}
