...
export default function SettingsPanel({preferences,onPreferences,rubriqueNoms,onRubriqueNoms,customRubriques,onCustomRubriques}:{preferences:Preferences;onPreferences:(p:Preferences)=>void;rubriqueNoms:RubriqueNoms;onRubriqueNoms:(r:RubriqueNoms)=>void;customRubriques:CustomRubrique[];onCustomRubriques:(r:CustomRubrique[])=>void}){
  ...
  const colors=['#1478ff','#7c3aed','#db2777','#059669','#ea580c','#0f766e']
  ...
  function addRubrique(){
    ...
    onCustomRubriques([...customRubriques,{id:crypto.randomUUID(),titre:newTitle.trim(),sousTitre:newSub.trim()}])
    setNewTitle('');setNewSub('')
  }
  function editRubrique(r:CustomRubrique){
    const titre=window.prompt('Titre de la rubrique :',r.titre)
    if(!titre?.trim())return
    const sousTitre=window.prompt('Sous-titre :',r.sousTitre)
    if(sousTitre===null)return
    onCustomRubriques(customRubriques.map(x=>x.id===r.id?{...x,titre:titre.trim(),sousTitre:sousTitre.trim()}:x))
  }
  function deleteRubrique(r:CustomRubrique){
    if(window.confirm(`Supprimer la rubrique « ${r.titre} » ?`))onCustomRubriques(customRubriques.filter(x=>x.id!==r.id))
  }
  ...
  return <section className="panel settings-panel">
    ...
    <div className="setting-block"><strong>Couleur principale</strong><div className="color-choices">{colors.map(c=><button key={c} className={`color-choice ${preferences.accent===c?'selected':''}`} style={{background:c}} onClick={()=>setPref('accent',c)} aria-label={`Couleur ${c}`}/>)}</div></div>
